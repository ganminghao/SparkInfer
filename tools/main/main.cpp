...
static void bench_perf_print(const std::vector<bench_perf_data> & data) {
    ...
    if (data.size() > 1 && total_prefill_ms > 0.0 && total_decode_ms > 0.0) {
        ...
        LOG("\nTotal (excluding warmup, SKIP for decode = 0):\n");
...
static std::vector<std::string> bench_load_prompts(const std::string & path, int n_prompts) {
    ...
    while (std::getline(fin, line) && (n_prompts == -1 || count < n_prompts)) {
        ...
        prompts.push_back(std::move(processed));
        ++count;
    }
...
static int bench_main(int argc, char ** argv) {
    ...
    for (size_t prompt_idx = 0; prompt_idx < prompts.size(); ++prompt_idx) {
        ...
        // reset KV cache & perf for this prompt
        ...
        for (int i = 0; i < (int) embd_inp.size();) {
            ...
            if (n_past + n_eval >= n_ctx) {
                int max_eval = n_ctx - n_past;
                if (max_eval <= 0) {
                    LOG_WRN(
                        "%s: prompt %zu prefill hit context limit (n_past = %d, n_ctx = %d), "
                        "stopping prefill early\n",
                        __func__, prompt_idx, n_past, n_ctx);
                    break;
                }
                LOG_WRN(
                    "%s: prompt %zu prefill truncated to avoid context overflow "
                    "(n_eval %d -> %d, n_past = %d, n_ctx = %d)\n",
                    __func__, prompt_idx, n_eval, max_eval, n_past, n_ctx);
                n_eval = max_eval;
            }
        ...
        while (n_remain != 0) {
            if (n_past + 1 >= n_ctx) {
                LOG_WRN(
                    "%s: prompt %zu decode hit context limit (n_past = %d, n_ctx = %d), "
                    "stopping early\n",
                    __func__, prompt_idx, n_past, n_ctx);
                break;
            }
