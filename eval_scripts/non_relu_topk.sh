...
if [[ -z $MODE || $MODE == "low" ]]; then
    run_block "pwif=0.0001" topk.yaml non_relu_logs non_relu_topk low "${MODELS_LOW[@]}"
fi

if [[ -z $MODE || $MODE == "high" ]]; then
    run_block "pwif=0.0001" topk.yaml non_relu_logs non_relu_topk high "${MODELS_HIGH[@]}"
fi
