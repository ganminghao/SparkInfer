            ...
            case LLM_ARCH_BAMBOO:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (params.use_sparkinfer) {
                            ...
                            layer.ffn_pred_up     = create_tensor(tn(LLM_TENSOR_FFN_PRED_UP, "weight", i), {n_embd, n_pred_lora[i]}, TENSOR_NOT_REQUIRED);
                            layer.ffn_pred_down   = create_tensor(tn(LLM_TENSOR_FFN_PRED_DOWN, "weight", i), {n_pred_lora[i], n_ff}, TENSOR_NOT_REQUIRED);
                            layer.ffn_pred_up_b   = create_tensor(tn(LLM_TENSOR_FFN_PRED_UP, "bias", i), {n_pred_lora[i]}, TENSOR_NOT_REQUIRED);
                            layer.ffn_pred_down_b = create_tensor(tn(LLM_TENSOR_FFN_PRED_DOWN, "bias", i), {n_ff}, TENSOR_NOT_REQUIRED);
                            layer.ffn_down        = create_tensor(tn(LLM_TENSOR_FFN_DOWN, "weight", i), {n_ff,   n_embd}, 0);
                        }
